...
export async function setSecureItem(key: string, value: string): Promise<void> {
  try {
    ...
    throw new Error(`Failed to securely store item: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getSecureItem(key: string): Promise<string | null> {
  try {
    ...
    if (!encrypted || !salt || !iv || !Array.isArray(encrypted) || !Array.isArray(salt) || !Array.isArray(iv)) {
